...
input_string = """turn off 660,55 through 986,197
...
if __name__ == "__main__":
    start_now = time.time_ns()
    total = 0
    test = Lights()
    test.make_grid((1000, 1000))

    lines = input_string.replace("turn", "").split("\n")
    _actions = []
    ...
    for action in _actions:
        current_action, start, end = action
        if current_action == "on":
            test.turn_on(start, end)
        if current_action == "off":
            test.turn_off(start, end)
        if current_action == "toggle":
            test.toggle(start, end)

    for v in test.pixels.values():
        if v == 1:
            total += 1
    print(total)

    end_now = time.time_ns()

    time_total = end_now - start_now
    print(time_total)

    # test.turn_on((0, 0), (2, 5))
    # test.show_grid()
